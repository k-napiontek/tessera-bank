...
Twenty-five of the plan's twenty-nine rows are `Done` - WP-10, WP-11 and WP-24 each count more than
once, all being split in the plan - and **stratum 1 is complete**. Three of the four that remain are
the rest of the **workload strand**, WP-24b, WP-24c and WP-25: WP-20 declared the bank day as a
contract, WP-21 drives it at the estate, WP-22 gave it a ledger with a year of postings in it to
drive at, WP-23 said what good looks like and recorded what normal actually is, and WP-24a made the
estate breakable on purpose.
...
| **Workload** | Mostly built. The model, the driver, a production-shaped ledger and a recorded normal all exist: a compressed bank day runs against the modern spine, the queries behind it meet a year of postings rather than three fixture accounts, every component that emits a metric states what good looks like for it, and `workload/baselines/` holds what the estate actually did with the manifest and dataset digest that produced it. The audit chain's throughput ceiling is now a measured figure rather than a hunch - and adding a second ledger instance does not move it. The estate can now be degraded on purpose too: a scenario is a versioned contract ([ADR 0017](docs/governance/adr/0017-a-scenario-is-its-own-contract.md)), an injector applies one at the minute it names, and a run report judges what happened against what the scenario declared would. What remains is the seven recorded signatures (WP-24c), a schema migration under traffic and a soak (WP-24b), and the older strata driven from the same model (WP-25). |
