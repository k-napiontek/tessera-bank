...
Twenty-one of the plan's twenty-seven rows are `Done` - WP-10 and WP-11 each count twice, both being
split in the plan - and **stratum 1 is complete**. Three of the six that remain are the rest of the
**workload strand**, WP-23 to WP-25; WP-20 declared the bank day as a contract, WP-21 drives it at
the estate, and WP-22 gave it a ledger with a year of postings in it to drive at.
...
| **Workload** | Half-built. The model, the driver and a production-shaped ledger exist: a compressed bank day runs against the modern spine, and the queries behind it now meet a year of postings rather than three fixture accounts. What remains is an SLO catalogue with a recorded baseline (WP-23), failure injection and a soak (WP-24), and the older strata driven from the same model (WP-25). Until WP-23 there is no recorded normal, so a figure from a run is an observation rather than an objective. |
